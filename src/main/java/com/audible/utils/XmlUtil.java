  ...
  public static List<Table> getTables(String datebaseFilename) throws Exception {
    ...
    for (int temp = 0; temp < item.getLength(); temp++) {
      ...
      if (nNode.getNodeType() == Node.ELEMENT_NODE) {
        ...
        table.setPostgresSql(eElement.getElementsByTagName("POSTGRES_SQL").item(0).getTextContent());
        table.setOracleSql(eElement.getElementsByTagName("ORACLE_SQL").item(0).getTextContent());
        tables.add(table);
      }
    }
